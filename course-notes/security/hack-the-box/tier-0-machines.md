...
- VM - Virtual Machine
- Terminal - used to interact with the operating system in order to issue
  commands via the command line, such as the one to start our VPN connection;
  also known as the console or shell
- Hack the Box labs use the OpenVPN service to form its VPN connections
- `tun` - the abbreviated name for a tunnel interface in the output of a VPN
  boot-up sequence
...
- `nmap -sV {target_IP} -Pn` will scan for services and version on ports and
  override ping probes being blocked
- anonymous - the username that is used over FTP when you want to log in
...
- `redis-cli -h {target_IP}` can be used to connect to the Redis database on
  the host IP (assuming that database is using the default port)
- Once connected to the Redis server, `info` will display information and
