...
- `Get-Help` displays information about a cmdlet*.* To get help with a
  particular command, run the following: `Get-Help Command-Name`
- Pass the `-examples` flag to commands to understand exactly how to use
  the command
- `Get-Command` gets all the cmdlets \*\*installed on the current Computer
  - It allows for pattern matching like the following: `Get-Command Verb-*`
    or `Get-Command *-Noun`

#### Object Manipulation

- cmdlets return objects (which contain methods and properties); to
  manipulate the output, there are 2 options:
